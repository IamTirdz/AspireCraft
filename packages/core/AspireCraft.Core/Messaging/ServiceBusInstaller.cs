...
public sealed class ServiceBusInstaller : IPackageInstaller
{
    public bool CanInstall(ProjectConfiguration configuration)
    {
        return configuration.Integrations.Contains(IntegrationType.ServiceBus);
    }
    ...
    public void Install(ProjectConfiguration configuration, TemplateContext context)
    {
        ...
        var output = Path.Combine(context.RootPath, "src", directory, "Messaging", "AzureMessagingBusService.cs");
